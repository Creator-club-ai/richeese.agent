...
핵심 워크플로우는 5개 core skill로 운영한다. 각 단계는 사용자 승인 없이 다음 major step으로 넘어가지 않는다.
...
source-intake packet을 바탕으로 콘텐츠를 끝까지 설계한다. 단일 방향, working title, core thesis, save reason, 슬라이드 흐름, role / goal / key point를 함께 정하고 `editor`가 바로 카피를 쓸 수 있는 상태로 넘긴다. 승인 시 Obsidian `03_brief/`에 저장한다.

### `editor` — 에디터
승인된 콘텐츠 기획안을 받아 각 슬라이드의 headline과 body 원고를 쓴다. 승인 시 Obsidian `04_copy/`에 저장한다.

### `designer` — 디자이너
승인된 원고와 콘텐츠 기획안 분류를 받아 디자인 노트를 작성한다. 사용자 최종 승인 후 Obsidian `05_handoff/`와 프로젝트 루트 `final_report.md`에 저장한다.
...
- 평상시에는 `feed-fetcher`를 쓴다.

## Human-in-the-Loop Workflow

```
뉴스 플로우:
feed-fetcher → [신호 선택] → source-intake → [wiki ingest 완료 / source packet 확인] → content-planner → [기획 승인] → editor → [원고 승인] → designer → [최종 승인] → final_report.md

직접 소스 플로우:
source-intake → [wiki ingest 완료 / source packet 확인] → content-planner → [기획 승인] → editor → [원고 승인] → designer → [최종 승인] → final_report.md
```
...
- 원칙은 `한 포스트 = 한 카드`다. 카드만 시각화하고, content plan / copy / handoff는 카드 본문에서 링크로 연결한다.
- 운영 대시보드는 workflow 단계보다 `Latest News / Planned Content / Publish Date / Priority`를 먼저 본다.
- `03_brief/` ~ `05_handoff/`는 stage storage이고, 운영 홈에서는 전면에 노출하지 않는다.
...
- System-owned memory artifacts save to Obsidian automatically: append-only `raw/`, `오늘의 뉴스/`, `06_wiki/`.
- User-approved stage outputs save to Obsidian: `03_brief/`, `04_copy/`, `05_handoff/`.
