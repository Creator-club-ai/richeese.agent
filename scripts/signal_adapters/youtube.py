...
from .common import MAX_PER_FEED, SignalArticle, SourceConfig, YOUTUBE_LOOKBACK, is_relevant, run_ytdlp_json


def fetch_youtube_channel(config: SourceConfig) -> list[SignalArticle]:
    articles: list[SignalArticle] = []
    cutoff = datetime.now(timezone.utc) - timedelta(hours=YOUTUBE_LOOKBACK)
...
def fetch_youtube_search(config: SourceConfig) -> list[SignalArticle]:
    ...
    items = run_ytdlp_json(["--flat-playlist", search_term], timeout=60)
    if not items:
